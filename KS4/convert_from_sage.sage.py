...
# This file was *autogenerated* from the file convert_from_sage.sage
...
import os
import numpy as np
top_files = [d for d in os.listdir('top_data') if d[_sage_const_0 ] == 'h' and 'topdata.p' in d]
print('num files', len(top_files))
assert_mismatch = []
global count_all
def top_info_from_file(file):
    h11 = int(file.split("_")[_sage_const_1 ])
    poly = int(file.split("_poly")[_sage_const_1 ].split("_topdata")[_sage_const_0 ])
...
#     s[s.index('y')+1:])
    ...
    data = pickle.load(f)
    f.close()
    verts0 = data[_sage_const_1 ]
    verts1  = [[int(j) for j in vert] for vert in verts0]
    genera0 = data[_sage_const_2 ]
    genera1  = [int(j) for j in genera0]
    dualdim0 = data[_sage_const_3 ]
