...
def batch_upper_left(tensor,m): # in: (batch_size,1,n,n) out: upper left (batch_size, 1, m, m)
    return torch.Tensor.narrow(torch.Tensor.narrow(tensor,2,0,m),1,0,m)
...
def ensemble_eigenvalues(data_tensor, h11): # data_tensor is batch of matrices
...
def gen_matrices(generator, h11, args, noise = None):
    labels = torch.tensor([[1.0 if k == h11 - 1 else 0.0 for k in range(args.max_h11)] for _ in range(args.test_batch_size)])

    if type(noise) == type(None):
        noise = torch.FloatTensor(args.test_batch_size, args.nz)
        noise.resize_(args.test_batch_size, args.nz).normal_(0, 1)
    
    return generator(noise, labels), labels

def test_generator(generator,h11,args,real_eigs,plus_log_eigs=True):
    #print(datetime.datetime.now())
    fakes, labels = gen_matrices(generator,h11,args)
    #print(datetime.datetime.now())
    eigs = ensemble_eigenvalues(fakes, h11)
    ...
    if plus_log_eigs:
        ...
        return wasserstein_distance(eigs, real_eigs), wasserstein_distance(log_eigs, log_real_eigs), eigs, log_eigs
    else:
...
def show_GAN_histogram(GAN, epoch, h11, args, real_eigs, nz = 100, dpi=500, noise = None, inverse_wishart = False, batchSize = 10000, kde = False, display_wishart = True, norm_hist = True, show = True, log10 = True, ylim = None, xlim = None):
    ...
    data_tensor = gen_matrices(GAN, h11, args, noise=noise)
    #data_tensor = batch_upper_left(data_tensor, h11)
